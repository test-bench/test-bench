    ...
    context "Assert Block" do
      context "Nesting" do
        ...
        test do
          ...
          Finished test "Example failure" (Result: failure)
            #{caller_location_1}: Assertion failed (TestBench::Fixture::AssertionFailure)
              Inner Context
                Comment #1
                Finished test "Inner test (pass)" (Result: pass)
                Finished test "Inner test (failure)" (Result: failure)
                  #{caller_location_2}: Assertion failed (TestBench::Fixture::AssertionFailure)
                    Deep Inner Context
                      Comment #3
                      Finished test "Deep inner test (pass)" (Result: pass)
                      Finished test "Deep inner test (failure)" (Result: failure)
