...
+assert+ can take an optional block containing test code. All the tests performed by the block must pass in order to satisfy the assertion. By default, any test output produced by the block is printed only when the assertion fails, allowing the block to convey useful details about the failure. The purpose of this block-form syntax is to allow for specialized assertions. In fact, +assert_raises+ and +refute_raises+ are themselves specialized assertions that both make use of the block-form of +assert+ in order to provide information about what error was expected, what error (if any) was raised, and more.

NOTE: When a block is passed to +assert+, a positional argument _must not_ be passed along with it.
