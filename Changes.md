...
### 2.0.0.6 - Mon Aug 28 2023

* CLI Arguments
  - Added `--require-passing-tests` and `--no-require-passing-tests` (and `-p` and `-P`, respectively)
    - Controls whether the exit status of the CLI should be nonzero when there isn't a single test that passes
    - Requiring a single passing test is the default, but `--no-require-passing-tests` deactivates the default
    - Can also be controlled via `TEST_BENCH_REQUIRE_PASSING_TESTS`
